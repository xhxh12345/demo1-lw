...
class frame1:
    def __init__(self, root):
        ...
        fontStyle2 = tkFont.Font(family="Lucida Grande", size=30)
        # 标题
        title = tk.Label(self.frame1, text='登 录Sky_AGI ', font=fontStyle2)
        # 用户名输入
        label1 = tk.Label(self.frame1, text='username: ', font=fontStyle)

        entry1 = tk.Entry(self.frame1, textvariable=self.username, font=fontStyle)
        # 密码输入
        label2 = tk.Label(self.frame1, text='password: ', font=fontStyle)
        entry2 = tk.Entry(self.frame1, textvariable=self.password, font=fontStyle)
        # 功能按钮
        button1 = tk.Button(self.frame1, text='确认', font=fontStyle, command=self.login)
...
class frame3:
    def __init__(self, root):
        ...
        title = tk.Label(self.frame3, text='Ai视频分析系统', font=fontStyle)
        # 画布
        canvas1 = tk.Canvas(self.frame3, width=1728, height=972, bg='white')
        # notebook1
        ...
        # 功能按钮
        tkinter.Button(self.frame3, text='退出',command=self.login_frame2).grid(column=17, row=4)
        tkinter.Button(self.frame3, text='发布web').grid(column=17, row=5)
        tkinter.Button(self.frame3, text='发布到屏幕2', command=self.login_frame5).grid(column=17, row=6)
...
class frame4:
    def __init__(self, root):
        ...
        canvas1 = tk.Canvas(self.frame4, width=1728, height=972, bg='white')
        # notebook1
        frameOne = tkinter.Frame()
        frameTwo = tkinter.Frame()
...
class frame5:
    def __init__(self, root):
        ...
        # 所有控件
        fontStyle = tkFont.Font(family="Lucida Grande", size=20)
        title = tk.Label(self.frame5, text='Ai视频分析系统',font = fontStyle )
        canvas1 = tk.Canvas(self.frame5, width=1536, height=864, bg='white')
        label1 = tk.Label(self.frame5, text='Ai分析结果显示')
